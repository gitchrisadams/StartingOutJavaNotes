package com.christopheradams.num002WritingOurOwnDotEqualsMethodForRectClass;

/**
	Rectangle Class, phase 1
	Under construction.
*/


public class Rectangle
{
	private double length;
	private double width;


	/**
		Constructor with no args
	*/
	public Rectangle()
	{
		length = 1.0;
		width = 1.0;
	}


	/**
		Constructor
		@param len The length of the rectangle.
		@parm w The width of the rectangle.
	*/
	public Rectangle(double len, double w)
	{
		// Instead of setting fields directly, we call
		// the setter/mutator passing in len/w to set the length and width.
		// Now we take advantage of validation in the setter.
		setLength(len);
		setWidth(w);
	}

	/*
		The setLength method stores a value in the length field.
		@param len The value to store in the length.
	*/
	public void setLength(double len)
	{
		// Store the len value passed to setLength, in the class field length as
		// long as len is greater or equal to zero.
		if(len >= 0)
			length = len;
		else
			// Display error and don't set len. We could also just leave the else off
			// and value never gets set and no error displays.
			System.out.println("Bad value: " + len);
	}


	/**
		The setWidth method stores a value in the width field.
		@parm w The value to store in width.
	*/
	public void setWidth(double w)
	{
		// Set the width field in the Rectangle class to w passed in
		// as long as w is greater or equal to zero.
		// If it is greater set the width, otherwise don't set it
		// and don't display an error either.
		if(w >= 0)
			width = w;
	}


	/**
		The getLength method returns a Rectangle object's length.
		@return The value in this Class's length field.
	*/
	public double getLength()
	{
		// Return the Rectangle Class length field to calling function
		return length;
	}


	/**
		The getWidth method returns a Rectangle object's width.
		@return The value in this Class's width field.
	*/
	public double getWidth()
	{
		// Return the Rectangle Class width field to calling function
		return width;
	}


	/**
			The getArea method returns a Rectangle object's area.
			@return The product of length times width.
		*/
		public double getArea()
		{
			// Return the length * the width.
			return length * width;
	}

	// Our own .equals() method that tells if
	public boolean equals(Rectangle r)
	{
		// Initially set boolean to false.
		boolean isEqual = false;

		// Set boolean to true if they are equal.
		if(length == r.length && width == r.width)
			isEqual = true;

		return isEqual;
	}
}